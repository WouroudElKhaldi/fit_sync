...
const EmailVerification: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (code.trim().length !== 6) {
      setErrorMsg('Please enter your 6-digit email verification code.');
      return;
    }
  ...
  const handleModalConfirm = () => {
    ...
    // After email is verified, navigate back to Login so user can sign in
    navigate('/login');
  };
    ...
    >
      ...
      <div className="w-full max-w-[450px] z-10 flex flex-col gap-6">
        ...
        <div className="flex flex-col items-center gap-2 text-center">
          <span className="material-symbols-outlined text-primary text-5xl font-light">mark_email_read</span>
          <h1 className="text-3xl font-black tracking-tight text-white drop-shadow-md">FITSYNC PRO</h1>
          <p className="text-sm font-semibold text-white/70 drop-shadow-sm">Confirm Account Authenticity</p>
        </div>
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs uppercase tracking-wider font-bold text-white/70 pl-1 text-center">Verification Code</label>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-[100] p-4">
          ...
          >
            ...
            <div>
              <h3 className="text-2xl font-black tracking-tight text-on-surface">Email Verified!</h3>
              <p className="text-sm text-on-surface-variant/60 leading-relaxed mt-2">
                Congratulations! Your account has been verified. You can now use your credentials to sign in.
              </p>
            </div>
